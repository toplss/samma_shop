...
<html>
    ...
    <body bgcolor="#FFFFFF" text="#242424" leftmargin=0 topmargin=15 marginwidth=0 marginheight=0 bottommargin=0 rightmargin=0>
        ...
        try {
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                $signKey = "eTd5cnVMUzhRVVlMS1NUVE9wM2JYdz09";
	...
	if($_REQUEST['resultCode'] == "0000"){
		// 결제결과 DB처리
		$mode = $_REQUEST['mode'];
		$bank_account = $resultMap['vactBankName'].' '.$resultMap['VACT_Num'].' '.$resultMap['VACT_InputName'];

		/*
		if($mode == 'charge_order_virtual') {
			echo "<meta http-equiv='Refresh' content='0; URL=/?sub=mypage&pg=charge_point_proc_query&mode=$_GET[mode]&trade_code=$_GET[trade_code]&no=$_GET[no]&order_number=$_GET[order_number]&bank_account=$bank_account&trans_number=$resultMap[tid]&result_code=$_REQUEST[resultCode]'>";
		} else if($mode == 'charge_order_card' || $mode == 'charge_order_cellphone' || $mode == 'charge_order_bank_transfer') {
			echo "<meta http-equiv='Refresh' content='0; URL=/?sub=mypage&pg=charge_point_proc_query&mode=$_GET[mode]&trade_code=$_GET[trade_code]&no=$_GET[no]&order_number=$_GET[order_number]&trans_number=$resultMap[tid]&result_code=$_REQUEST[resultCode]'>";
		}
		*/

		if($mode == 'charge_order_virtual') {
			echo "<meta http-equiv='Refresh' content='0; URL=/?sub=sub&pg=charge_amount_proc_query&mode=$_GET[mode]&trade_code=$_GET[trade_code]&no=$_GET[no]&order_number=$_GET[order_number]&bank_account=$bank_account&trans_number=$resultMap[tid]&result_code=$_REQUEST[resultCode]'>";
		} else if($mode == 'charge_order_card' || $mode == 'charge_order_cellphone' || $mode == 'charge_order_bank_transfer') {
			echo "<meta http-equiv='Refresh' content='0; URL=/?sub=sub&pg=charge_amount_proc_query&mode=$_GET[mode]&trade_code=$_GET[trade_code]&no=$_GET[no]&order_number=$_GET[order_number]&trans_number=$resultMap[tid]&result_code=$_REQUEST[resultCode]'>";
		}
		// 결제결과 DB처리
	} else {
		//echo ("<script>alert('결제에 실패했습니다.'); opener.document.location.href='/?sub=mypage&pg=charge_point'; window.close();</script>");
		echo ("<script>alert('결제에 실패했습니다.'); opener.document.location.href='/?sub=sub&pg=charge_amount'; window.close();</script>");
	}
